...
这个项目用于在玩家自己的游戏目录中生成并安装双语字幕：普通字幕保留英文上一行、简体中文下一行；带时间片段的对白会合成同一行 `English / 简中`，减少游戏同时显示多条字幕时的错层。V1 版本已经可用于本地测试和日常游玩。
...
PS：你也看到了，除了这里，没一句话是我写的，安详.jpg。

## V1 效果

- 带 `{*1}`、`{*2}` 等时间片段的对白显示为 `English / 简体中文`，优先保证英中同步
- 普通静态文本仍使用 `English line + {B} + 简体中文`，游戏内表现为英文上一行、简中下一行
